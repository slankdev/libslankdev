...
print_mac (struct sr_mac *mac)
{
  printf ("Mac:\t\t%02x:%02x:%02x:%02x:%02x:%02x\n",
	  ...
	  (unsigned char) mac->oct[4], (unsigned char) mac->oct[5]);
}

int
is_esadecimal (char c)
{
  int i, ret = 0;
...
usage_srdev (void)
{
  fprintf (stderr, "Usage: srconf srdev { help | flush } \n\n");
  return 0;
}
...
int
add_end_dx2 (int argc, char **argv)
{
  int ret = -1;
  if (argc > 6)
    {
...
int
add_end_dx4 (int argc, char **argv)
{
  int ret = -1;
...
int
add_end_ad6 (int argc, char **argv)
{
  int ret = -1;
  struct in6_addr next_hop;
...
int
add_end_ad4 (int argc, char **argv)
{
  int ret = -1;
  struct in_addr next_hop;
...
int
add_end_as6 (int argc, char **argv)
{
...
int
add_end_as4 (int argc, char **argv)
{
  printf ("The behaviour  %s is not supported yet. \n", argv[4]);
  return 0;
}
